...
approach considering constant acceleration in an interval $[t_k, t_{k+1})$:
\begin{align}
...
\begin{equation}
    \bm{\ddot{p}}_{t,k} = \bm{R}(\bm{\hat{x}}_{k}) \tensor[^t]{\bm{a}}{_{t,k}}
\end{equation}

\noindent with:
\begin{equation}
    \bm{R}(\bm{\hat{x}}_{k}) = \bm{R}_z(\gamma) \bm{R}_y(\beta) \bm{R}_x(\alpha)
\end{equation}
...
\section{Walking to a desired position}
Todo.
...
[TODO: SAY WHICH ARE THE VALUES USED IN THE IMPLEMENTATION - CAMERA TRILATERATION].

Figures \ref{fig:comp-ground-truth-estimated-torso}
%and \ref{fig:diff_ground_truth_estimated_torso}
for the
...
(figure \ref{fig:walk_to_desired_position}).
...
\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{images/walk_to_desired_position.png}
    \caption{Trajectory followed by the robot when walking to the desired position
        $\bm{p}_g = (-3, 5)$. Here the measurements of the Kalman filter are obtained
        using the accelerometer and the gyroscope. In green the ground truth of the coordinates $(x, y)$ of the
        robot, in red the estimate of the Kalman filter. The robot start at the
        position $\bm{p}_s$ and it stops at the position $\bm{p}_f$.}
