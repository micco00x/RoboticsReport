...
\begin{align}
    ...
    \dot{\gamma}_r &= \frac{sin(\gamma_r)}{\rho}v - w \\
    \dot{\delta}_r &= \frac{sin(\gamma_r)}{\rho}v
\end{align}
...
and \ref{eq:orientation-gyro-integration}. Results are shown in figure
...
\begin{equation}
    \bm{\dot{\hat{p}}}_{t,k} \approx \bm{\dot{\hat{p}}}_{t,k-1}
    = \frac{\bm{\hat{p}}_{t,k} - \bm{\hat{p}}_{t,k-1}}{T}
\end{equation}

This approximation, as it is possible to see from figure
\ref{fig:comp_ground_truth_estimated_torso_accelerometer_prevlinearvelocity},
improves the previous result, estimating correctly the coordinates $(x, y)^T$
and reducing the error on $z$. This suggests that another approach is needed
