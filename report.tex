...
Another way to obtain the same goal is to exploit the reference frame
...
Given the estimate of the pose of the torso $\bm{\hat{x}}$ we checked that it
could be used to estimate correctly also the position of the CoM of the robot
$\bm{\hat{p}}_{CoM}$ with respect to the world frame (eq.
\ref{eq:pose-rf-com-world}). The results are shown in Fig.
\ref{fig:comp_estimated_torso_com}. We then expressed the position of the CoM
...
when using gains $k_1=0.18$, $k_2=0.014$ makes robot stops at the final
...
\begin{figure}
    ...
    \caption{Trajectory followed by the robot when moving to a desired configuration
        $\bm{q}_g = (-2, 3.2, \pi)$ using the posture regulation task
...
\begin{figure}
    ...
    \caption{Linear and angular velocity of the HRP4 while performing
        ...
        as explained in subsection \ref{subsec:posture_regulation},
        when $\rho_r < 0.2$, $v$ are $w$ are set to 0.}
...
localize in more complex environment, enabling advanced behaviours and increasing
