...
the data obtained from the IMU. The measurements of the pose of the
...
Considering constant acceleration in an interval $[t_k, t_{k+1})$:
\begin{align}
    v_{t, k+1} &= v_{t, k} + a_{t, k} T \\
    p_{t, k+1} &= p_{t, k} + v_{t, k} T + \frac{1}{2} a_{t, k} T^2
\end{align}
...
\begin{figure}
    ...
    \caption{Trajectory followed by the robot when walking to the desired position
        $p_g = (-3, 5)$. Here the measurements of the Kalman filter are obtained
        ...
        position $p_s$ and it stops at the position $p_f$.}
